(define (domain mydomain)
    ...
    (:action stack
        ...
        :precondition (and (holding ?r ?b1) (free ?b2) (not (on ?b2 ?b1)))
        :effect (and (not (holding ?r ?b1)) (not (on-table ?b1)) (on ?b1 ?b2))
    )

    (:action unstack
        :parameters (?r - roboticarm ?b1 - block ?b2 - block)
        :precondition (and (holding ?r ?b1) (on ?b1 ?b2))
        :effect (and (not (holding ?r ?b1)) (on-table ?b1) (not (on ?b1 ?b2))
    )
)